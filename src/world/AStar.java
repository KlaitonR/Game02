	...
	public static List<Node> findPath(World world, Vector2i start, Vector2i end){
		...
		while(openList.size() > 0) {
			...
			for(int i = 0; i < 9; i++) {
				...
				int xi = (i%3) - 1;
				int yi = (i/3) - 1;
				Tile tile = Game.world.tiles[x+xi+((y+yi)*Game.world.WIDTH)];
				if(tile == null) continue;
				if(tile instanceof WallTile) continue;
				if(i == 0) {
					Tile test = Game.world.tiles[x+xi+1+((y+yi) * Game.world.WIDTH)];
					Tile test2 = Game.world.tiles[x+xi+((y+yi+1) * Game.world.WIDTH)];
					...
					Tile test = Game.world.tiles[x+xi+((y+yi-1) * Game.world.WIDTH)];
					Tile test2 = Game.world.tiles[x+xi+1+((y+yi) * Game.world.WIDTH)];
						if(test instanceof WallTile || test2 instanceof WallTile) {
							continue;
						}
				}
